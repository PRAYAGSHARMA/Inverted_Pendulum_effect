...
function eqbm_points = find_equilibrium_points(x1_dot, x2_dot)
  ...
  equations = [x1_dot == 0; x2_dot == 0]
  eqbm_points = solve(equations);
...
function jacobian_matrices = find_jacobian_matrices(eqbm_points, x1_dot, x2_dot)
  ...
  Jacobian = jacobian([x1_dot, x2_dot], [x1, x2]);                            # The Jacobian for the set of equations x1_dot and x2_dot
  for point = 1:length(eqbm_points)
    jacobianAtEqmbPt = double(subs(Jacobian, [x1, x2], solutions{point}));    # double() to convert a complex sym to floating point  
    jacobian_matrices{point} = jacobianAtEqmbPt;
  endfor
...
function [eigen_values stability] = check_eigen_values(eqbm_pts, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    eigen = eigen_values{k};
    for index = 1:length(eigen)
      if real(eigen(index)) > 0      # real() to return real part in each entry in eigen
        flag = 0;
      endif
    endfor
